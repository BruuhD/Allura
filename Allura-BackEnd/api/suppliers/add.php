...
if (empty($name) || empty($ruc) || empty($gender) || empty($product)) {
    sendResponse(false, "Campos obligatorios incompletos");
}

try {
    $stmt = $pdo->prepare("
        INSERT INTO suppliers 
        (name, commercial_name, gender, product, ruc, phone, email, address, bank_name, active, created_at, updated_at)
    ...
    sendResponse(true, "Suplidor agregado correctamente");
} catch (Exception $e) {
    sendResponse(false, "Error al agregar suplidor: " . $e->getMessage());
}
?>
