<?php
// ==========================================
// 📊 Reporte de Ventas y Clientes - Allura
// ==========================================
...
// ==========================================
// 🧾 2️⃣ Historial de Ventas (Agrupado por número de orden)
// ==========================================
$sqlHistorialAgrupado = "
SELECT 
  o.order_number,
  o.proveedor AS cliente,
  o.total,
  o.abono,
  o.estado,
...
// ==========================================
// 🧾 3️⃣ Historial de Ventas por Cliente
// ==========================================
$sqlPorCliente = "
SELECT 
  proveedor AS cliente,
...
WHERE order_type = 'venta'
  AND created_at BETWEEN :desde AND :hasta
GROUP BY cliente, rnc
ORDER BY monto_total DESC;
";
$stmt = $pdo->prepare($sqlPorCliente);
$stmt->execute([':desde' => $fecha_desde, ':hasta' => $fecha_hasta]);
$resPorCliente = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
SELECT 
  i.product_name,
  SUM(i.quantity) AS total_vendido,
  SUM(i.quantity * i.price) AS ganancia_total
FROM orders o
...
LIMIT 1;
";
$stmt = $pdo->prepare($sqlProductoTop);
$stmt->execute([':desde' => $fecha_desde, ':hasta' => $fecha_hasta]);
$productoTop = $stmt->fetch(PDO::FETCH_ASSOC);

// ==========================================
// 🖨️ Generación del PDF
// ==========================================
$pdf->SetFont("Arial", "B", 16);
$pdf->Cell(0, 10, utf8_decode("REPORTE DE VENTAS Y CLIENTES - ALLURA"), 0, 1, "C");
...
$pdf->Cell(0, 8, utf8_decode("1. Cuentas por Cobrar"), 0, 1);
$pdf->SetFont("Arial", "", 11);
$pdf->MultiCell(0, 6, utf8_decode("Muestra las ventas pendientes de pago por parte de los clientes. Incluye el total, abono actual y saldo restante."));
$pdf->Ln(5);
...
foreach ($resPendientes as $row) {
    $pdf->Cell(45, 8, utf8_decode($row["cliente"]), 1);
...
// ==========================================
// 🔹 Sección 2: Historial de Ventas agrupadas
// ==========================================
$pdf->SetFont("Arial", "B", 13);
$pdf->Cell(0, 8, utf8_decode("2. Historial de Ventas por Número de Orden"), 0, 1);
$pdf->SetFont("Arial", "", 11);
$pdf->MultiCell(0, 6, utf8_decode("Resumen de ventas agrupadas por orden, mostrando totales y unidades sin detallar cada producto."));
$pdf->Ln(5);
...
// ==========================================
$pdf->SetFont("Arial", "B", 13);
$pdf->Cell(0, 8, utf8_decode("3. Historial de Ventas por Cliente"), 0, 1);
$pdf->SetFont("Arial", "", 11);
$pdf->MultiCell(0, 6, utf8_decode("Resumen agrupado por cliente, mostrando montos totales, abonos y saldos pendientes."));
$pdf->Ln(5);
...
$pdf->Cell(30, 8, "Ventas", 1, 0, "C", true);
$pdf->Cell(30, 8, "Total", 1, 0, "C", true);
$pdf->Cell(30, 8, "Abonado", 1, 0, "C", true);
$pdf->Cell(30, 8, "Pendiente", 1, 1, "C", true);

$pdf->SetFont("Arial", "", 10);
foreach ($resPorCliente as $row) {
    $pdf->Cell(50, 8, utf8_decode($row["cliente"]), 1);
    $pdf->Cell(30, 8, utf8_decode($row["rnc"]), 1);
    $pdf->Cell(30, 8, $row["total_ventas"], 1, 0, "C");
...
// ==========================================
// 🔹 Sección 4: Abonos de Clientes
// ==========================================
$pdf->SetFont("Arial", "B", 13);
$pdf->Cell(0, 8, utf8_decode("4. Abonos de Clientes"), 0, 1);
$pdf->SetFont("Arial", "", 11);
$pdf->MultiCell(0, 6, utf8_decode("Relación de ventas con abonos realizados. Incluye el total del pedido, monto abonado, saldo restante y estado actual."));
$pdf->Ln(5);
...
// ==========================================
// 🔹 Sección 5: Top Productos Más Vendidos
// ==========================================
$sqlTopProductos = "
SELECT 
  i.product_name,
...
if ($topProductos) {
    $pdf->SetFont("Arial", "B", 13);
    $pdf->Cell(0, 8, utf8_decode("5. Top Productos Más Vendidos"), 0, 1);
    $pdf->SetFont("Arial", "", 11);
    $pdf->MultiCell(0, 6, utf8_decode("Listado de los productos con mayor volumen de ventas durante el período seleccionado, ordenados de mayor a menor cantidad."));
    $pdf->Ln(5);
    ...
    $pdf->Cell(45, 8, "Ganancia Total", 1, 1, "C", true);

    // Datos
    $pdf->SetFont("Arial", "", 10);
    $rank = 1;
    foreach ($topProductos as $row) {
        // Resaltar el primer lugar con color de fondo distinto
        if ($rank === 1) {
            $pdf->SetFillColor(255, 245, 200); // dorado suave
            $fill = true;
        } else {
        ...
        $pdf->Cell(40, 8, $row["total_vendido"], 1, 0, "C", $fill);
        $pdf->Cell(45, 8, "RD$ " . number_format($row["ganancia_total"], 2), 1, 1, "R", $fill);
