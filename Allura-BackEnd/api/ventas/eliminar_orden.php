...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT id, nfc_code 
        FROM orders 
        WHERE order_number = :order_number AND order_type = 'venta'
    ");
    $stmt->execute([":order_number" => $order_number]);
    $order = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$order) {
        echo json_encode(["status" => false, "msg" => "La orden de venta no existe."]);
        exit;
    }
    ...
    echo json_encode([
        "status" => true,
        "msg" => "Orden de venta eliminada correctamente y código NFC liberado."
    ]);
