...
try {
    ...
    $cliente = null;
    if (!empty($rnc)) {
        $check = $pdo->prepare("SELECT id, product FROM suppliers WHERE ruc = :ruc LIMIT 1");
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES (:order_number, :nfc_code, :proveedor, :rnc, :telefono, :total, 'Pendiente', 'compra')
    ");
    $stmt->execute([
        ...
        foreach ($items as $it) {
            ...
            foreach ($productosLimpios as $producto) {
                ...
                $insertCli = $pdo->prepare("
                    INSERT INTO suppliers (name, product, ruc, phone, active, created_at, updated_at)
