...
$stmtOrder = $pdo->prepare("
    SELECT 
        order_number,
        CAST(nfc_code AS CHAR) AS nfc_code,  -- ✅ evita pérdida de precisión
        proveedor,
        rnc,
        telefono,
        DATE(created_at) AS fecha
    FROM orders
    WHERE order_number = ? AND order_type = 'compra'
");
...
$stmtItems = $pdo->prepare("
    ...
    FROM order_items
    WHERE order_id = (SELECT id FROM orders WHERE order_number = ? AND order_type = 'compra')
");
...
$pdf->Cell(0, 10, utf8_decode("ORDEN DE COMPRA (ACTUALIZADA)"), 0, 1, "C");
$pdf->Ln(3);
...
$pdf->SetFont("Arial", "", 11);
$pdf->Cell(100, 6, utf8_decode("Proveedor: ") . ($order["proveedor"] ?? ""), 0, 0);
