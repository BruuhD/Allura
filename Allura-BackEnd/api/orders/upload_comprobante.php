...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, total, abono, estado FROM orders WHERE order_number = ? AND order_type = 'compra'");
    ...
    if (strtolower($order['estado']) === 'pagado') {
        throw new Exception("La orden ya está pagada completamente.");
    }
    ...
    echo json_encode([
        "status" => true,
        "msg" => "Comprobante de compra guardado correctamente.",
